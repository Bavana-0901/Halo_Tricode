...
import { Upload, FileText, Sparkles, AlertCircle, ArrowRight, ShieldCheck, Image as ImageIcon } from 'lucide-react';
...
export default function UploadView({ onAnalyze, loading, error }) {
  ...
  const supportedFormats = ".pdf,.doc,.docx,.txt,.png,.jpg,.jpeg,.webp";
  ...
  const getFileIcon = (filename) => {
    if (!filename) return <FileText className="w-12 h-12 mb-3 text-slate-500" />;
    const ext = filename.split('.').pop().toLowerCase();
    if (['png', 'jpg', 'jpeg', 'webp'].includes(ext)) {
      return <ImageIcon className="w-12 h-12 mb-3 text-indigo-400" />;
    }
    return <FileText className="w-12 h-12 mb-3 text-indigo-400" />;
  };
    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="glass-panel rounded-2xl p-6 flex flex-col justify-between border border-slate-800 hover:border-indigo-500/30 transition-all">
            <div>
              ...
              <p className="text-xs text-slate-400 mb-4">
                Supported formats: <strong className="text-slate-200">PDF, DOC, DOCX, TXT, PNG, JPG, WEBP</strong>.
              </p>
            ...
            <div>
              ...
              <div className="flex p-1 bg-slate-900 rounded-lg mb-4">
                <button
                  ...
                  className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all ${jdMode === 'text' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  Paste Text
                </button>
                <button
                  ...
                  className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all ${jdMode === 'file' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  Upload File
                </button>
              </div>
              ...
              {jdMode === 'text' ? (
                ...
                <label className="relative flex flex-col items-center justify-center border-2 border-dashed border-slate-700 hover:border-indigo-500 rounded-xl p-8 cursor-pointer bg-slate-900/50 hover:bg-slate-900/80 transition-all h-[172px]">
                  <input 
                    type="file" 
                    accept={supportedFormats}
                    ...
                    className="hidden" 
                  />
                  {getFileIcon(jdFile?.name)}
                  {jdFile ? (
                    <div className="text-center">
                      <p className="text-sm font-medium text-white max-w-[200px] truncate">{jdFile.name}</p>
                      <p className="text-xs text-indigo-400 mt-1">{(jdFile.size / 1024).toFixed(1)} KB • Ready</p>
                    </div>
                  ) : (
                    <div className="text-center">
                      <p className="text-sm font-medium text-slate-300">Click or drag JD here</p>
                    </div>
                  )}
                </label>
              )}
        ...
        <div className="text-center pt-4">
          <button
            type="submit"
            disabled={!file || (jdMode === 'text' ? !jdText.trim() : !jdFile) || loading}
            className={`inline-flex items-center justify-center gap-3 px-8 py-4 rounded-xl text-base font-bold text-white shadow-xl shadow-indigo-500/20 transition-all duration-200 ${
              !file || (jdMode === 'text' ? !jdText.trim() : !jdFile) || loading
          ...
          >
            {loading ? (
              <>
                ...
                <span>Analyzing Compatibility & Extracting Data...</span>
              </>
            ) : (
              <>
